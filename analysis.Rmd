...
subj_data <- raw_data %>%
  ...
  pivot_longer(cols = c(accuracy, rt), names_to = "measure")
...
```{r plot-1, fig.width = 5, fig.height = 4}
...
```{r complex-analysis}
chose_complex_data <- raw_data %>%
  filter(test_part == "box_selection") %>%
  mutate(chose_complex = case_when(
    utt == "complex" ~ accuracy,
    T ~ 1 - accuracy)) %>%
  pivot_longer(cols = c(chose_complex, rt), names_to = "measure")

chose_complex_mean_data <- chose_complex_data %>%
  group_by(measure, boxes_type, utt) %>%
  tidyboot_mean(value)
```

```{r plot-complex, fig.width = 5, fig.height = 4}
ggplot(chose_complex_mean_data %>% filter(measure == "chose_complex"),
       aes(fill = utt, x = boxes_type, y = empirical_stat)) +
  geom_bar(position="dodge", stat="identity") +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), 
                width = 0.2, position=position_dodge(.9)) + 
...
ggplot(chose_complex_mean_data %>% filter(measure == "rt"),
       aes(fill = utt, x = boxes_type, y = empirical_stat)) +
  geom_bar(position="dodge", stat="identity") +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), 
                width = 0.2, position=position_dodge(.9)) + 
