...
```{r unique workers}
dfiles <- list.files(here("mturk/novel4objects/production-results/"), pattern = "*.json", full.names = T)
...
write_csv(workers, here("mturk/novel4objworkers.csv"))
...
for(i in 1:length(ldf)){
    ...
    tmp <- read.csv(textConnection(tmp_trials))
    tmp_data <- tmp %>%
        select(c(rt, button_pressed, test_part, utt_1, utt_2, box, 
                 obj, responses)) %>%
        rename(left_utt = utt_1, right_utt = utt_2) %>%
        mutate(subid = as.numeric(i),
           responses = str_remove_all(responses, "[\"{Q}0:]"),
           obj = str_remove(obj, "img/"),
           left_utt = as.character(left_utt),
           right_utt = as.character(right_utt),
           #left_obj = str_remove(left_obj, "img/"),
           #right_obj = str_remove(right_obj, "img/"),
           selected_utt = if_else(test_part == "obj_selection", 
                     ifelse(button_pressed == "1", right_utt, left_utt), ""),
...
write_csv(jsdata, here("data/new4_data.csv"))
