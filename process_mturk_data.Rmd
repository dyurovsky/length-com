...
```{r unique workers}
dfiles <- list.files(here("mturk/3objects/production-results/"), pattern = "*.json", full.names = T)
...
write_csv(workers, here("mturk/3objworkers.csv"))
...
#filenames <- list.files(here("mturk/novel4objects/production-results/"), pattern = "*.json", full.names = T)
#filenames <- list.files(here("mturk/new4objects/production-results/"), pattern = "*.json", full.names = T)
filenames <- list.files(here("mturk/novel3objects/production-results/"), pattern = "*.json", full.names = T)
...
for(i in 1:length(ldf)){
    ...
    tmp <- read.csv(textConnection(tmp_trials))
    tmp_data <- tmp %>%
        select(c(rt, button_pressed, test_part, utt, 
                 obj_1, obj_2, obj_3, responses)) %>%
        #rename(left_utt = utt_1, right_utt = utt_2) %>%
        mutate(subid = as.numeric(i),
           responses = str_remove_all(responses, "[\"{Q}0:]"),
           obj_1 = str_remove(obj_1, "img/"),
           obj_2 = str_remove(obj_2, "img/"),
           obj_3 = str_remove(obj_3, "img/"),
           utt = as.character(utt),
           selected_obj = if_else(test_part == "obj_selection", 
                     ifelse(button_pressed == "0", obj_1,
                            ifelse(button_pressed == "1", obj_2, obj_3)), ""),
           accuracy = if_else(test_part == "obj_selection",
                          if_else(
                                utt == "complex_utt" & selected_obj == "simplenovel.png" |
                                utt == "complex_utt" & selected_obj == "simplenovel2.png" |
                                utt == "simple_utt" & selected_obj == "complexnovel2.png", 
                                1, 0), 0)
           )
      #new_data <- tmp_data %>%
...
#write_csv(jsdata, here("data/novel4_data.csv"))
write_csv(jsdata, here("data/novel3_data.csv"))
